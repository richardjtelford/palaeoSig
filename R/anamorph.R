...
`anamorph` <- function (x, k, plot = FALSE) {
  ...
  low <- predict(lm(x.g.low ~ gaus.low), 
                 newdata = data.frame(gaus.low = -10))
  high <- predict(lm(x.g.high ~ gaus.high), 
                  newdata = data.frame(gaus.high = 10))
  r <- range(x)
  if (plot) {
    plot(c(-10, gaus, 10), c(low, x.g, high),  type="l", 
