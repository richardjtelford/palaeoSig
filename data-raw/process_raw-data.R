...
if(!file.exists("data-raw/MARGO_Atlantic_modern_PF.xls")) {
  download.file(url = "http://store.pangaea.de/Projects/MARGO/regionaldata/MARGO_Atlantic_modern_PF.xls",
                destfile = "data-raw/MARGO_Atlantic_modern_PF.xls")
}

N_Atlantic <- read_excel(path = "data-raw/MARGO_Atlantic_modern_PF.xls", 
                         sheet = "North Atlantic", skip = 1) %>%
  mutate(`Longitude (decimal, from -180 to +180)` = as.numeric(`Longitude (decimal, from -180 to +180)`), 
         `Water depth (m)` = as.numeric(`Water depth (m)`)) 

S_Atlantic <- read_excel(path = "data-raw/MARGO_Atlantic_modern_PF.xls", 
                         sheet = "South Atlantic", skip = 1) 
  

Atlantic <- bind_rows(N_Atlantic, 
                      S_Atlantic %>% filter(!Core %in% N_Atlantic$Core)#remove overlap
                      ) %>%
  select(Core:`Total Planktics`) %>% 
  select(-`Coring device`, 
         -`Water depth (m)`,
         -(Ocean:`date of addition`), 
         -(`Other ID`:`Total Planktics`)) %>% 
  rename(Longitude = `Longitude (decimal, from -180 to +180)`,
...
#remove rare taxa
Atlantic <- Atlantic %>% gather(key = taxon, value = percent, -(Core:Longitude)) %>% 
  filter(percent > 0) %>% 
  group_by(taxon) %>% 
  filter(n() > 50) %>%  
  ungroup() %>% 
...
if(!file.exists("data-raw/woa13_decav_t13mn01v2.csv.gz")) {
  download.file(url = "http://data.nodc.noaa.gov/woa/WOA13/DATAv2/temperature/csv/decav/1.00/woa13_decav_t13mn01v2.csv.gz",
                destfile = "data-raw/woa13_decav_t13mn01v2.csv.gz")
}
if(!file.exists("data-raw/woa13_decav_t14mn01v2.csv.gz")) {
  download.file(url = "http://data.nodc.noaa.gov/woa/WOA13/DATAv2/temperature/csv/decav/1.00/woa13_decav_t14mn01v2.csv.gz",
                destfile = "data-raw/woa13_decav_t14mn01v2.csv.gz")
}
if(!file.exists("data-raw/woa13_decav_t15mn01v2.csv.gz")) {
  download.file(url = "http://data.nodc.noaa.gov/woa/WOA13/DATAv2/temperature/csv/decav/1.00/woa13_decav_t15mn01v2.csv.gz",
                destfile = "data-raw/woa13_decav_t15mn01v2.csv.gz")
}
if(!file.exists("data-raw/woa13_decav_t16mn01v2.csv.gz")) {
  download.file(url = "http://data.nodc.noaa.gov/woa/WOA13/DATAv2/temperature/csv/decav/1.00/woa13_decav_t16mn01v2.csv.gz",
...
Atlantic$summ50 <- interp::interp(
  ...
  output = "points", xo = Atlantic$Longitude, yo = Atlantic$Latitude)$z
...
arctic.pollen <- arctic %>% select(starts_with("F-")) %>% as.data.frame()
arctic.env <- arctic %>% select(-matches("^[FC]-")) %>% as.data.frame()
